...
```
curl -X POST http://localhost:8080/signin -d '{"username":"admin","password":"password"}' | export TOKEN=`jq '.token' | tr -d '"'`

curl -X POST http://localhost:8080/signin -d '{"username":"mlabouardy","password":"L3nSFRcZzNQ67bcc"}' | export TOKEN=`jq '.token' | tr -d '"'`

curl  -X GET http://localhost:8080/recipes -H "Authorization: Bearer ${TOKEN}"
```
